...
using System.IO;
using UnityEngine;
using HVR.iOS.Editor.Xcode.PBX;

namespace HVR.iOS.Editor.Xcode
{
    ...
    public class PBXProject
    {
        ...
        /// This can be changed by setting the <c>frameworkLocation</c> parameter.
        /// </summary>
        ...
        /// <param name="frameworkLocation">The location of the framework on the computer. Usually System/Library/Frameworks.</param>
        public bool ContainsFramework(string targetGuid, string framework, string frameworkLocation = "System/Library/Frameworks", PBXSourceTree tree = PBXSourceTree.Sdk)
        {
            var fileGuid = FindFileGuidByRealPath(frameworkLocation + framework, tree);
            if (fileGuid == null)
                return false;
        ...
        /// <param name="frameworkLocation">The location of the framework on the computer. Usually System/Library/Frameworks.</param>
        public void AddFrameworkToProject(string targetGuid, string frameworkName, bool weak, string frameworkLocation = "System/Library/Frameworks/", PBXSourceTree tree = PBXSourceTree.Sdk)
        {
            string fileGuid = AddFile(frameworkLocation + frameworkName, "Frameworks/" + frameworkName, tree);
        ...
        /// <param name="framework">The name of the framework. The extension of the filename must be ".framework".</param>
        /// <param name="frameworkLocation">The location of the framework on the computer. Usually System/Library/Frameworks.</param>
        public void RemoveFrameworkFromProject(string targetGuid, string framework, PBXSourceTree tree)
        {
            var fileGuid = FindFileGuidByRealPath(framework, tree);
            if (fileGuid == null)
                return;
        ...
        */
        ...
        { 
            ...
            // Ensure the framework has the correct properties and attributes set
            PBXElementDict props = embedFrameworkFileData.GetPropertiesRaw();

            PBXElementDict settings = null;

            if (props.Contains("settings"))
                settings = props["settings"].AsDict();
            else
                settings = props.CreateDict("settings");
            ...
            // These attributes are required for embedded frameworks.
            attrs.AddString("CodeSignOnCopy");
            attrs.AddString("RemoveHeadersOnCopy");

            embedFrameworkFileData.SetPropertiesWhenSerializing(props);
