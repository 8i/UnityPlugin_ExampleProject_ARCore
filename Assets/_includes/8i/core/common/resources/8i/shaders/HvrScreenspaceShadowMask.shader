...
/////////////////////////////////////////////////////
// 8i modified version:
// Mostly based on Unity 2018.10f2 built-in shader Hidden/Internal-ScreenSpaceShadows but with blend mode changed.
Shader "8i/Hidden/Internal-ScreenSpaceShadows" {
Properties {
...
// Turn below on if we can actually re-use _ShadowMapTexture and _ShadowMapTexture_TexelSize;
...
// 8i's additional
float4 _ShadowFadeCameraDistance;
...
fixed4 frag_hard (v2f i) : SV_Target
{
    ...
    float fade = saturate(vpos.z / _ShadowFadeCameraDistance.r);
    fade = 1.0 - pow(fade, _ShadowFadeCameraDistance.g);
...
// NOTE: It has to change to use shader pass instead of subshader as we only have access to specify pass in command buffer.
SubShader {
    
    // ----------------------------------------------------------------------------------------
    // Pass for hard shadows:
    // Just collect shadows into the buffer. Used on pre-SM3 GPUs and when hard shadows are picked.
    // TODO: test required for stereo instancing(UNITY_STEREO_INSTANCING_ENABLED), STEREO_CUBEMAP_RENDER_ON, OpenGL, MacOS
    // TODO: test required for 2017.x version

    Pass {
        Tags{ "ShadowmapFilter" = "HardShadow" }
        ZWrite Off ZTest Always Cull Off
        Blend DstColor Zero // need to multiply with previous color, all the subshaders follow this blend mode.
    ...
    Pass {
        ...
        ZWrite Off ZTest Always Cull Off
        Blend DstColor Zero // need to multiply with previous color
